...
public class OrderManageService {
    ...
    public CommonResult order(OrderVo vo) {
        ...
        Map<Long, ProductResult> productMap = productList.getData().stream().collect(Collectors.toMap(ProductResult::getId, Function.identity()));
        //准备生成的订单
        ...
        vo.getProductOrders().forEach(it -> {
            ...
            ProductResult productResult = productMap.get(it.getProductId());
            ...
            orderEntity.setPrice(productResult.getSpecialPrice()*it.getNumber());
            orderEntity.setUserId(vo.getId());
            ProductSnapObject productSnapObject = new ProductSnapObject();
            BeanUtils.copyProperties(productResult,productSnapObject);
